...
{
    class DefaultStrategyWorldGenerator : WorldGenerator
    {
        ...
        public DefaultStrategyWorldGenerator(EntityHierarchyScenario scenario)
        {
            m_Scenario = scenario;
        }

        protected override World GenerateWorld()
        {
            var world = new World("DefaultGroupingStrategy Setup");
            var entityManager = world.EntityManager;

            var allEntities = GenerateEntitiesWithFragmentation(entityManager, Allocator.TempJob);
        ...
        static readonly ProfilerMarker k_GenerateAllEntitiesMarker = new ProfilerMarker($"{nameof(DefaultStrategyWorldGenerator)} : Generate All Entities");
        ...
        NativeArray<Entity> GenerateEntitiesWithFragmentation(EntityManager entityManager, Allocator allocator)
        {
            using (k_GenerateAllEntitiesMarker.Auto())
            {
                ...
                var allEntities = new NativeArray<Entity>(m_Scenario.TotalEntities, allocator);

                // Burst-friendly running counter, used when copying new entities to allEntities array
                var entityIndex = new NativeReference<int>(0, Allocator.TempJob);
                ...
                for (int archetypeIndex = 0; archetypeIndex < archetypesToCreate; ++archetypeIndex)
                {
                    using (k_GenerateArchetypeBatchMarker.Auto())
                    {
                        ...
                        var createdEntities = entityManager.CreateEntity(archetype, entitiesToAdd, Allocator.TempJob);
        ...
        static readonly ProfilerMarker k_AssignParentsMarker = new ProfilerMarker($"{nameof(DefaultStrategyWorldGenerator)} : Assign Parents");
        ...
        void AssignParents(NativeArray<Entity> allEntities, EntityManager entityManager)
        {
            using (k_AssignParentsMarker.Auto())
            {
                ...
                var depthMap = new NativeArray<UnsafeList<Entity>>(m_Scenario.MaximumDepth, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
                ...
                depthMap[0] = new UnsafeList<Entity>(numberAtRoot, Allocator.TempJob) { m_length = numberAtRoot };
                depthMap[depthCount - 1] = new UnsafeList<Entity>(numberAtMaxDepth, Allocator.TempJob) { m_length = numberAtMaxDepth };

                for (int i = 1; i < depthCount - 1; ++i)
                {
                    depthMap[i] = new UnsafeList<Entity>(numberAtOtherDepths, Allocator.TempJob) { m_length = numberAtOtherDepths };
                }
        ...
        struct SetFragmentId : IJob
        {
            ...
            public void Execute()
            {
                ...
                {
                    TransactionManager.SetSharedComponentData(Entities[i], new SegmentId { Value = i % SegmentsCount });
                }
            }
        }
        ...
        struct AssignEntityGuid : IJobParallelFor
        {
            ...
            public void Execute(int index)
                => CommandBuffer.SetComponent(index, Entities[index], new EntityGuid(Entities[index].Index, 0, (uint)index));
        }
        ...
        struct DistributeParents : IJob
        {
            ...
            public void Execute()
            {
                ...
                for (;i < parentsAtRoot.Length; ++i)
                {
                    ...
                    // Debug.Log($"[{i}] Add {Entities[i]} at depth 1 (root) ({i+1}/{parentsAtRoot.length})");
                }
                ...
                for (var depthIndex = 1; depthIndex < depthCount; ++depthIndex)
                {
                    ...
                    for (int d = 0; d < currentDepth.Length; ++d)
                    {
                        ...
                        // Debug.Log($"[{i}] Add {entity} at depth {depthIndex+1} ({d+1}/{currentDepth.length}) with parent {DepthMap[depthIndex-1][parentIndex]} (parent: {parentIndex+1} / {DepthMap[depthIndex-1].Length})");
                    }
